...
public:
    ...
    void fsmkdir(std::string path_){
        if(fs::exists(path_)){
            std::string command = "rm -r " + std::string(path_);  
    ...
    template<typename CloudTPtr>
    void cloudDownSample(const CloudTPtr& cloud_, CloudTPtr& cloudD_){
        pcl::ApproximateVoxelGrid<pcl::PointXYZI> sample;
        sample.setLeafSize(1.0, 1.0, 1.0);
        sample.setInputCloud(cloud_);
        sample.filter(*cloudD_);
    }
